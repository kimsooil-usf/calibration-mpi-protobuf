...
//  auto maskJSON = readJSONFile(GLOBAL.input_base + "maskwearing_November10_2022.json");
  auto maskJSON = readJSONFile(GLOBAL.input_base +GLOBAL.mask_filename);// mask wearing from filename;
...
//   double temp_non_compliance_metric = 0;
   ...
   for (auto &elem: maskJSON.GetArray()){
 mask[index].day = elem["day"].GetInt();

  mask[index].maskcompliance=elem["maskcompliance"].GetDouble();

// 	mask[index].mask_compliance=static_cast<OfficeType>(elem["day"].GetInt());;

// 	//Cyclic strategy class
// 	if(GLOBAL.CYCLIC_POLICY_ENABLED && GLOBAL.CYCLIC_POLICY_TYPE == Cycle_Type::home){
// 	  mask[index].cyclic_strategy_class = uniform_count_type(0, GLOBAL.NUMBER_OF_CYCLIC_CLASSES - 1);
// 	}

//     mask[index].age_independent_mixing = 0;
// 	if(GLOBAL.USE_AGE_DEPENDENT_MIXING){
// 	  mask[index].age_dependent_mixing.resize(GLOBAL.NUM_AGE_GROUPS, 0);
// 	}
	++index;
  }
   return mask;
	  ...
	  if((elem.HasMember("num_days")) && (elem["num_days"].GetInt() > 0)){
		...
		if(elem.HasMember("neighbourhood_containment")){
		  temp.neighbourhood_containment = elem["neighbourhood_containment"]["active"].GetBool() 
		  	&& GLOBAL.ENABLE_NEIGHBORHOOD_SOFT_CONTAINMENT
			&& GLOBAL.ENABLE_NBR_CELLS;
		  if(!(GLOBAL.ENABLE_NEIGHBORHOOD_SOFT_CONTAINMENT && GLOBAL.ENABLE_NBR_CELLS)){
			std::cout<<std::endl<<"To enable containment strategies, add  --ENABLE_NEIGHBORHOOD_SOFT_CONTAINMENT and --ENABLE_NBR_CELLS to argument list. Ignoring neighbourhood containment.";
		  }else{
			if(elem["neighbourhood_containment"].HasMember("leakage")){
				temp.neighbourhood_containment_leakage = elem["neighbourhood_containment"]["leakage"].GetDouble();
			}else{
				temp.neighbourhood_containment_leakage = GLOBAL.LOCKED_NEIGHBORHOOD_LEAKAGE;
			}
			if(elem["neighbourhood_containment"].HasMember("threshold")){
				temp.neighbourhood_containment_threshold = elem["neighbourhood_containment"]["threshold"].GetDouble();
			}else{
				temp.neighbourhood_containment_threshold = GLOBAL.NEIGHBORHOOD_LOCK_THRESHOLD;
			}
		  }
		}
		if(elem.HasMember("ward_containment")){
		  ...
		  if(!GLOBAL.ENABLE_CONTAINMENT){
			...
			if(elem["ward_containment"].HasMember("leakage")){
				temp.ward_containment_leakage = elem["ward_containment"]["leakage"].GetDouble();
			}else{
				temp.ward_containment_leakage = GLOBAL.LOCKED_COMMUNITY_LEAKAGE;
			}
			if(elem["ward_containment"].HasMember("threshold")){
				temp.ward_containment_threshold = elem["ward_containment"]["threshold"].GetDouble();
			}else{
				temp.ward_containment_threshold = GLOBAL.COMMUNITY_LOCK_THRESHOLD;
			}
...
//-----------Gender---------------------------
      //categories are : 1=male, 2=female
	nodes[i].gender= elem["gender"].GetInt();

//--------------------------------------------
//---------------Poverty/income level----------
// income categories (based on PolicyMap 2016-2020 annual income)
// 1: <$25000
// 2: $25000 ~ $34999
// 3: $35000 ~ $49999
// 4: $50000 ~ $74999
...
// 7: $125000 ~ $149999
// 8: >$150000
	nodes[i].income= elem["income"].GetInt(); 

//------------------Race-----------------------
// race (based on PolicyMap)
// 1: White
// 2: African American
// 3: Asian
...
// 7: Two or more races
	...
	nodes[i].race= elem["race"].GetInt();
//-------------------------------------

//----------------Ethnicity----------------------
//1: hispanic, 2=non-hispanic (maybe including don't know and not declare)


	nodes[i].ethnicity= elem["ethnicity"].GetInt();//
//-----------------------------------------------
#ifdef DEBUG
	...
	nodes[i].recovered_to_sususceptible_period = gamma(GLOBAL.RECOVERED_TO_SUSCEPTIBLE_PERIOD_SHAPE,
									                   GLOBAL.RECOVERED_TO_SUSCEPTIBLE_PERIOD_SCALE);

	nodes[i].recovered_to_sususceptible_period_vaccinated1 = gamma(GLOBAL.RECOVERED_TO_SUSCEPTIBLE_PERIOD_VACCINATED1_SHAPE,
									                               GLOBAL.RECOVERED_TO_SUSCEPTIBLE_PERIOD_VACCINATED1_SCALE);
	nodes[i].recovered_to_sususceptible_period_vaccinated2 = gamma(GLOBAL.RECOVERED_TO_SUSCEPTIBLE_PERIOD_VACCINATED2_SHAPE,
									                               GLOBAL.RECOVERED_TO_SUSCEPTIBLE_PERIOD_VACCINATED2_SCALE);

	nodes[i].recovered_to_sususceptible_period_waning = gamma(GLOBAL.RECOVERED_TO_SUSCEPTIBLE_PERIOD_WANING_SHAPE,
									                          GLOBAL.RECOVERED_TO_SUSCEPTIBLE_PERIOD_WANING_SCALE);

	nodes[i].recovered_to_sususceptible_period_boosted = gamma(GLOBAL.RECOVERED_TO_SUSCEPTIBLE_PERIOD_BOOSTED_SHAPE,
									                           GLOBAL.RECOVERED_TO_SUSCEPTIBLE_PERIOD_BOOSTED_SCALE);

	nodes[i].recovered_to_sususceptible_period_boosted2 = gamma(GLOBAL.RECOVERED_TO_SUSCEPTIBLE_PERIOD_BOOSTED2_SHAPE,
									                            GLOBAL.RECOVERED_TO_SUSCEPTIBLE_PERIOD_BOOSTED2_SCALE);
