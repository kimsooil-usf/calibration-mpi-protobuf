...
//-----------------------------------//
  ...
  if (GLOBAL.USE_AGE_DEPENDENT_MIXING){
	...
	node.lambda_incoming_higher.home = node.kappa_H_incoming
	  * homes[node.home].age_independent_mixing_higher
	  * node.hd_area_factor;
//-------lambda_incoming_higher for variants begins--shakir------//
...
//------lambda_incoming_higher for variants ends--shakir------//
	...
	if(node.workplace != WORKPLACE_HOME) {
	  ...
	  node.lambda_incoming_higher.work = (node.attending?1.0:GLOBAL.ATTENDANCE_LEAKAGE)*node.kappa_W_incoming
		* workplaces[node.workplace].age_independent_mixing_higher;
...
//------lambda_incoming_higher for variants ends--shakir------//
  ...
  node.lambda_incoming_higher.project =  (node.attending?1.0:GLOBAL.ATTENDANCE_LEAKAGE)*node.kappa_W_incoming
		* workplaces[node.workplace].projects[node.workplace_subnetwork].age_independent_mixing_higher;
...
//------lambda_incoming_higher for variants ends--shakir------//  
  ...
  if((cur_time % 2) && node.travels()){
	...
	node.lambda_incoming_higher.travel = GLOBAL.BETA_TRAVEL
	  * node.commute_distance
	  * travel_fraction_higher[1-0];//------Delete shakir later after verifying----//
...
#pragma omp parallel for default(none) shared(nodes, community, SIZE)			\
  reduction(+: errors,													\
			susceptible,unvaccinated,vaccinated1,vaccinated2,waning,boosted, hd_area_susceptible,							\
